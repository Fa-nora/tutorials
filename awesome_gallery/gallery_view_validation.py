    ...
    if _gallery_validator is None:
        try:
            with misc.file_open(
                ...
                _gallery_validator = etree.RelaxNG(
                    etree.parse(f)
                )
            ...
            _logger.error(
                "Could not load gallery RNG file: %s",
                e
            )
    ...
    if _gallery_validator.validate(arch):
        ...
        if res_model and res_model in kwargs.get('env', {}):
            ...
            if image_field and image_field not in Model._fields:
                _logger.error(
                    f"Field '{image_field}' does not exist "
                    f"on model '{res_model}'"
                )
                return False

            if tooltip_field and tooltip_field not in Model._fields:
                _logger.error(
                    f"Field '{tooltip_field}' does not exist "
                    f"on model '{res_model}'"
                )
                return False
